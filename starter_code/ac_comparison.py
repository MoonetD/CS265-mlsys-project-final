...
def apply_activation_checkpointing(model, ac_decisions=None, percentage=0.5):
    """
    ...
    # If no activations are marked for RECOMPUTE, apply checkpointing to a percentage of bottleneck blocks
...
def profile_batch_size(batch_size, device_str='cuda:0', memory_budget_gb=None):
    ...
    if memory_budget_gb is None:
        # Set memory budget to 70% of the peak memory without AC
        memory_budget_gb = peak_memory_without_ac * 0.7 / (1024**3)
    ...
    model_with_ac = apply_activation_checkpointing(model, ac_decisions)
...
def main():
    ...
    parser.add_argument('--memory-budget', type=float, default=None,
                        help='Memory budget for activation checkpointing in GB')

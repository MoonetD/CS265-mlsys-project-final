...
---
### Decision (Code)
[2025-05-14 12:42:00] - Modified subgraph extraction to use node ranks instead of name matching

**Rationale:**
The activation checkpointing implementation was failing with errors like "Warning: Could not find node for activation convolution_144 in the graph" because it was trying to find nodes by name matching. The node names in the FX graph generated by trace_model_for_ac didn't match the activation names from the GraphProfiler. By prioritizing matching by rank over name matching, we can make the node lookup more robust and reliable.

**Details:**
1. Modified `find_node_by_name` in `starter_code/graph_rewriter.py` to:
   - First try to find a node with the exact rank from activation_liveness
   - If that fails, try to find a node with the closest rank
   - Only as a last resort try to match by name

2. Modified `extract_subgraph_for_activation` to use a more robust approach for finding nodes:
   ...
   - Ensure that closest_creation_rank <= closest_last_fw_rank
   - Get all nodes with ranks between closest_creation_rank and closest_last_fw_rank

3. Enhanced `trace_model_for_ac` with additional debugging information:
   - Print the graph node count
   - Print the first 10 node names
   - Print the first 10 node ranks

These changes make the subgraph extraction more robust by relying on node ranks, which are more reliable than node names for identifying nodes in the graph.
---
### Decision (Code)
[2025-05-14 12:44:30] - Enhanced backward node lookup in graph rewriter for activation checkpointing

**Rationale:**
After fixing the subgraph extraction by using node ranks instead of name matching, we encountered another issue: the graph rewriter couldn't find the backward use nodes for the activations marked for recomputation. This was because it was looking for nodes with exact ranks from the activation_liveness dictionary, but these ranks don't match the ranks in the traced graph.

**Details:**
Modified `rewrite_graph_with_recomputation` in `starter_code/graph_rewriter.py` to:
1. Create a mapping of ranks to nodes for efficient lookup
2. Find the closest graph rank to the first_bw_use_rank from activation_liveness
